...
public class PostScoreRefreshJob implements Job, CommunityConstant {
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        BoundSetOperations setOperations = redisTemplate.boundSetOps(postScoreKey);
        if (setOperations.size() == 0) {
            logger.info("[任务取消] 没有需要刷新的帖子!");
            return;
        }

        logger.info("[任务开始] 正在刷新帖子分数, 待刷新帖子数量为: " + setOperations.size());
        while (setOperations.size() > 0) {
            Object pop = setOperations.pop();
    ...
    private void refresh(Integer postId) {
        ...
        wrapper.eq(DiscussPost::getId, postId);
