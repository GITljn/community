...
public class DisscussPostController implements CommunityConstant {
    ...
    public String addDiscussPost(String title, String content) {
        ...
        // 将发布的帖子通过消息队列异步存储到elasticsearch
    ...
    public String getDiscussPost(@PathVariable("discussPostId") int discussPostId,
                                 Model model, PageInfo page) {
        // 帖子
        DiscussPost post = discussPostService.getById(discussPostId);
        model.addAttribute("post", post);
        // 作者
        User user = userService.queryById(Integer.parseInt(post.getUserId()));
        ...
        Page<Comment> p = new Page<>(page.getCurrent(), page.getSize());
        LambdaQueryWrapper<Comment> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(Comment::getEntityType, ENTITY_TYPE_POST);
        wrapper.eq(Comment::getEntityId, discussPostId);
        commentService.page(p, wrapper);
        page.setPath("/discuss/detail/"+discussPostId);
        page.setTotalRows((int) p.getTotal());
        List<Comment> commentList = p.getRecords();
        ...
        for (Comment comment : commentList) {
            ...
            wrapper.eq(Comment::getEntityType, ENTITY_TYPE_COMMENT);
            wrapper.eq(Comment::getEntityId, comment.getId());
